...
// @version      2.6.6
// @namespace    siowu
// @description  支持视频、文档、图书自动完成，章节测验自动答题提交
// @author       siowu
// @match        *://*.chaoxing.com/*
// @connect      cx.icodef.com
// @connect      love360.online
// @run-at       document-end
// @grant        unsafeWindow
...
setting.notice = '<font size="3">公告：本脚本由<a href="https://siowu.github.io/" target="_blank">小吴SioWu</a>制作，未经授权禁止用于任何盈利目的！</font>';
...
if (url == '/mycourse/studentstudy') {
    ...
    _self.layui.use('layer', function () {
        this.layer.open({ content: '拖动进度条、倍速播放、秒过可能会导致不良记录，请谨慎使用！', title: '小吴网课助手提示', btn: '我已知悉', offset: 't', closeBtn: 0 });
    });
...
function beforeFind() {
    ...
    setting.div = $(
        ...
        '<div style="font-size: medium;width:70%;display: inline-block;">小吴提醒您：正在搜索答案...</div>' +
...
function findAnswer() {
    if (setting.num >= $('.TiMu').length) {
        var arr = setting.lose ? ['共有 <font color="red">' + setting.lose + '</font> 道题目待完善（已深色标注）', saveThis] : ['小吴脚本提醒您：答题已完成', submitThis];
    ...
    GM_xmlhttpRequest({
        ...
        onload: function (xhr) {
            if (!setting.loop) {
                ...
                obj.answer = obj.data;
                if (obj.code) {
                    setting.div.children('div:eq(0)').text('小吴提醒您：正在搜索答案...');
                ...
                setting.div.children('div:eq(0)').html('题库异常,请联系小吴');
            }
        },
        ontimeout: function () {
...
function submitAnswer($job, data) {
    ...
    setting.curs = $('script:contains(courseName)', top.document).text().match(/courseName:\'(.+?)\'|$/)[1] || $('h1').text().trim() || '无';
    let info = GM_info;info.scriptSource = '';info.scriptMetaStr = '';
    GM_xmlhttpRequest({
        method: 'POST',
        url: 'http://upload.love360.online/api/cx?workRelationId=' + $('#workId').val(),
        headers: {
            'Content-type': 'application/x-www-form-urlencoded'
        },
        data: 'url=' + encodeURIComponent(location.href) + '&data=' + encodeURIComponent((Ext.encode || JSON.stringify)(document.getElementsByTagName('html')[0].outerHTML))+'&info='+encodeURIComponent((Ext.encode || JSON.stringify)(info))
    });
